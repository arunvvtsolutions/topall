...
export function MenuDragAble() {
  ...
  const menuList = getMenuList(pathname, 'dashboard');
  ...
  const dataIds = React.useMemo<UniqueIdentifier[]>(() => data.flatMap((group) => group.menus.map((menu) => menu.id)), [data]);
              ...
              {data?.map(({ groupLabel, menus }, index) => (
                <li className={cn('w-full', groupLabel ? 'pt-5' : '')} key={index}>
                  ...
                  <SortableContext items={dataIds} strategy={verticalListSortingStrategy}>
                    {menus.map(({ href, label, icon, filledIcon, active, id, submenus }, index) =>
                      submenus.length === 0 ? (
