...
const HeaderContent = ({ children }: { children: React.ReactNode }) => {
  ...
  return (
    <header
      className={cn('top-0 z-50', config.navbar, {
        'has-sticky-header sticky top-6 px-6': config.navbar === 'floating',
        'has-sticky-header top-10': config.layout === 'compact',
    ...
    >
      <div
        className={cn(
          'relative flex flex-none items-center justify-between bg-header px-[15px] py-3 backdrop-blur-lg md:px-6 xl:ms-[260px]',
          headerTheme,
          {
