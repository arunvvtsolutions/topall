import React from 'react';
import { Input, InputProps } from '@/components/ui/input';
import { Search } from 'lucide-react';

// Define the InputColor type based on the allowed values in your UI library
type InputColor = 'default' | 'primary' | 'secondary' | 'accent' | 'info' | 'success' | 'warning' | 'danger';

// Create a type that includes all props from InputProps except 'color', and add our own color prop
type SearchInputProps = Omit<InputProps, 'color'> & {
  color?: InputColor;
  placeholder?: string;
  left?: boolean; // New prop to determine icon position
};

const SearchInput = React.forwardRef<HTMLInputElement, SearchInputProps>(
  ({ className, placeholder = 'Search...', color = 'default', left = false, ...props }, ref) => {
    return (
      <div className="relative">
        {/* Search Icon - Position based on `left` prop */}
        <Search
          className={`pointer-events-none absolute top-1/2 h-4 w-4 -translate-y-1/2 ${color} ${left ? 'left-3' : 'right-3'}`}
        />

        {/* Input Field - Adjust padding based on `left` prop */}
        <Input
          ref={ref}
          type="search"
          className={`${left ? 'pl-10 pr-3' : 'pl-3 pr-10'} ${className || ''}`}
          placeholder={placeholder}
          {...props}
          // @ts-expect-error - Input component expects a specific InputColor type
          color={color}
        />
      </div>
    );
  }
);

SearchInput.displayName = 'SearchInput';

export default SearchInput;
