...
interface IDialogProps {
  isOpen: boolean;
  title?: string | any;
  secondaryTitle?: string | any; // Secondary title
  description?: string;
        ...
        <DialogHeader>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex flex-grow flex-col">
              {/* Main title */}
              <DialogTitle className={`${className} text-left text-base font-semibold text-[#222222]`}>{title}</DialogTitle>
              {/* Secondary title */}
              {secondaryTitle && <p className="text-left text-sm font-medium text-[#6F6F6F] md:mt-1">{secondaryTitle}</p>}
            </div>
            {/* Secondary button */}
            {secondaryButton && (
              <div className={`${isMobile ? 'ml-auto' : 'w-full overflow-x-auto'} flex md:mr-5 md:justify-end`}>
                {secondaryButton}
              </div>
            )}
          </div>
