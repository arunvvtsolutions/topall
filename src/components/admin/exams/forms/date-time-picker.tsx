...
const DateTimePicker: React.FC<DateTimePickerProps> = ({ name, label, control, required = false, error }) => {
  ...
  function handleTimeChange(type: 'hour' | 'minute' | 'ampm', value: string, onChange: (date: Date) => void, currentDate: Date) {
                        ...
                        {Array.from({ length: 12 }, (_, i) => i + 1)
                          ...
                          .map((hour) => (
                            <Button
                              ...
                              onClick={() => handleTimeChange('hour', hour.toString(), field.onChange, field.value || new Date())}
                            >
                              {hour}
                            </Button>
                          ))}
                      ...
                      <div className="flex p-2">
                        {Array.from({ length: 12 }, (_, i) => i * 5).map((minute) => (
                          <Button
                            ...
                            className={cn(
                              'aspect-square shrink-0 text-[#020817] hover:bg-gray-200 hover:text-[#020817]',
                              field.value && field.value.getMinutes() === minute ? 'bg-[#020817] text-white' : 'bg-transparent'
                            )}
                            onClick={() =>
                              handleTimeChange('minute', minute.toString(), field.onChange, field.value || new Date())
                            }
                          >
                      ...
                      <div className="flex p-2">
                        {['AM', 'PM'].map((ampm) => (
                          <Button
                            ...
                            onClick={() => handleTimeChange('ampm', ampm, field.onChange, field.value || new Date())}
                          >
                            {ampm}
                          </Button>
                        ))}
