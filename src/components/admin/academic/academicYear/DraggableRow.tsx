...
//dnt kit
import { AcademicYearList } from '@/types';
import { useSortable } from '@dnd-kit/sortable';
import { CSS } from '@dnd-kit/utilities';
import { CSSProperties, memo } from 'react';

interface TableRowProps {
  row: AcademicYearList;
  index: number;
  showActions: boolean;
  draggable: boolean;
...
const DraggableRow = ({ row, index, draggable, onDelete, onEdit, onShow, showActions }: TableRowProps) => {
  ...
  return (
    ...
    >
      ...
      <TableCell className="font-inter pr-1 text-left text-sm font-normal !text-[#4B4B4B]">
        {row.standards?.name || '-'}
      </TableCell>
      <TableCell className="font-inter pr-1 text-left text-sm font-normal !text-[#4B4B4B]">{row.startDate || '-'}</TableCell>
      <TableCell className="font-inter pr-1 text-left text-sm font-normal !text-[#4B4B4B]">{row.endDate || '-'}</TableCell>
      {showActions && (
