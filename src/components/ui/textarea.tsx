  ...
  'flex flex-1 bg-background w-full min-h-[80px] rounded-md border border-solid   px-3 py-[10px] text-sm  file:border-0 file:bg-transparent file:text-sm file:font-medium  read-only:bg-background read-only:text-default-500 disabled:cursor-not-allowed disabled:opacity-50  transition duration-300 ',
  {
    variants: {
      color: {
        ...
        secondary:
          'border-secondary-300 text-default-700 focus:outline-none focus:border-secondary-400 disabled:bg-secondary/30 disabled:placeholder:text-secondary  placeholder:text-secondary-400',
...
const Textarea = React.forwardRef<HTMLTextAreaElement, TextareaProps>(({ className, color, defaultValue, ...props }, ref) => {
  return (
    <div className="w-full text-xs flex-1">
      <textarea className={cn(textareaVariants({ color }), className)} ref={ref} {...props}>
        {defaultValue}
      </textarea>
    </div>
  );
