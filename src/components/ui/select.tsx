...
const selectVariants = cva(
  ' w-full  px-3 h-10 text-sm flex [&>svg]:h-5 [&>svg]:w-5 !rounded-[8px] border border-input   justify-between items-center  read-only:bg-background  disabled:cursor-not-allowed disabled:opacity-50  transition duration-300 ',
  {
    variants: {
      color: {
        default:
...
const SelectTrigger = React.forwardRef<React.ElementRef<typeof SelectPrimitive.Trigger>, SelectTriggerProps>(
  ({ className, children, color, size, open, examination, primaryIcon, ...props }, ref) => (
    <SelectPrimitive.Trigger ref={ref} className={cn(selectVariants({ color, size }), className)} {...props}>
      {children}
      <SelectPrimitive.Icon asChild>
        {open ? (
          <div className={`text-[30px] ${primaryIcon ? 'text-primary' : 'text-[#4B4B4B]'}`}>
            <Icon icon={examination ? 'oui:arrow-up' : 'ic:round-arrow-drop-up'} fontSize={examination ? 23 : 30} />
          </div>
        ) : (
          <div className={`text-[30px] ${primaryIcon ? 'text-primary' : 'text-[#4B4B4B]'}`}>
            <Icon icon={examination ? 'oui:arrow-down' : 'ic:round-arrow-drop-down'} fontSize={examination ? 23 : 30} />
          </div>
        )}
...
>(({ className, ...props }, ref) => (
  <SelectPrimitive.Label ref={ref} className={cn('py-1.5 pr-2 ps-4 text-sm font-semibold', className)} {...props} />
));
...
const SelectItem = React.forwardRef<
  ...
  React.ComponentPropsWithoutRef<typeof SelectPrimitive.Item> & {
    disabledTick?: boolean;
  }
>(({ className, children, disabledTick, ...props }, ref) => (
  <SelectPrimitive.Item
    ref={ref}
    className={cn(
      'relative flex w-full cursor-default select-none items-center rounded-sm py-1.5 pe-2 ps-4 text-sm outline-none focus:bg-primary/5 focus:text-[white] data-[disabled]:pointer-events-none data-[disabled]:opacity-50',
      className
    )}
    {...props}
  >
